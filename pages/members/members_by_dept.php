...
$matches = array_filter($stripped_department_listing, 
	function($var) use ($get_dept) { 
		if (is_string($var)) {

			// cyu - modified 01-13-2015: checks carefully for the department letters, returns it if it's a match
			$mod_var = explode('|',$var);
			// cyu - modified 01-13-2015: remove the slashes and we'll be left with department letters, compare
			if (str_replace('\\', '', $get_dept) === $mod_var[1]) { return $var; }
		} else { 
			return 0; 
		} 
	});
